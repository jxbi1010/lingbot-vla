"""
Compute normalization statistics for Franka LeRobot-format data (16-D state/action).

Franka: 7 joints + 1 gripper per arm = 16-D (vs ALOHA 14-D).

train_path must point to the LeRobot dataset folder (containing meta/info.json), e.g.:
  /home/jianxin/jx_ws/franka_data/handover_mic-franka_clean_50

Usage:
    CUDA_VISIBLE_DEVICES=0 bash train.sh scripts/compute_norm_franka.py configs/norm/franka.yaml \\
        --data.train_path /home/jianxin/jx_ws/franka_data/handover_mic-franka_clean_50 \\
        --data.norm_path assets/norm_stats/franka_data_norm.json
"""
...
# Franka: 7 arm joints + 1 gripper per arm = 16-D (dual-arm)
FRANKA_STATE_DIM = 16
DEFAULT_TRAIN_PATH = "/home/jianxin/jx_ws/franka_data/handover_mic-franka_clean_50"
DEFAULT_NORM_PATH = "assets/norm_stats/franka_data_norm.json"
...
def compute_norm(dataset, task_id, batch_size, stats_dict, state_norm_keys, action_norm_keys, delta_norm, expected_state_dim=None):
    ...
    try:
        for batch in tqdm(data_loader, desc=f"Computing stats of {task_id}"):
            for key in state_norm_keys:
                ...
                if expected_state_dim is not None and values.shape[-1] != expected_state_dim:
                    raise ValueError(
                        f"Franka expects state dim {expected_state_dim}, got {values.shape[-1]} for key {key}"
                    )
            ...
            for key in action_norm_keys:
                ...
                if expected_state_dim is not None and values.shape[-1] != expected_state_dim:
                    raise ValueError(
                        f"Franka expects action dim {expected_state_dim}, got {values.shape[-1]} for key {key}"
                    )
                stats_dict[key].update(values.reshape(-1, values.shape[-1]))
    except Exception:
        success = False
...
def main():
    ...
    logger.info_rank0("Prepare data (Franka 16-D state/action)")

    train_path = args.data.train_path or DEFAULT_TRAIN_PATH
    ...
    try:
        success = compute_norm(
            ...
            action_norm_keys,
            delta_norm,
            expected_state_dim=FRANKA_STATE_DIM,
        )
    ...
    if success:
        ...
        output_path = Path(args.data.norm_path or DEFAULT_NORM_PATH)
